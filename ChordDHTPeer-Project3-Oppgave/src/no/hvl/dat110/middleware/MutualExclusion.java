...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		List<Message> mess = removeDuplicatePeersBeforeVoting();

		// multicast the message to activenodes (hint: use multicastMessage)

		multicastMessage(message,mess);

		// check that all replicas have replied (permission)

		if(areAllMessagesReturned(mess.size())){
			// if yes, acquireLock
			acquireLock();
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for(Message mess : activenodes){

			// obtain a stub for each node from the registry
			NodeInterface stub = Util.getProcessStub(mess.getNodeIP(),mess.getPort());
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				// if sender wins, acknowledge the message, obta in a stub and call onMutexAcknowledgementReceived()
	...
	public void multicastReleaseLocks(Set<Message> activenodes) throws RemoteException {
		...
		// iterate over the activenodes
		for(Message mess : activenodes){
			Util.getProcessStub(mess.getNodeIP(),mess.getPort()).releaseLocks();
			// obtain a stub for each node from the registry
